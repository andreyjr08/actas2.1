...
//---------computador---------------
    ...
    public function select_All($id){
        $pdo = $this->pdo;
        $sql = "SELECT A.PARA, A.CEDULA,A.DE,A.ASUNTO,C.COMPUTADOR, C.ACTIVO_FIJO,C.SERIAL,C.PROCESADOR,C.MEMORIA_RAM,C.SERIAL_CARGADOR FROM acta A INNER JOIN computador C ON A.COMPUTADOR_ID = $id AND C.ID = $id";
    ...
    public function select_computer($id){
       $pdo = $this->pdo;
     ...
     public function select_person($id){
        $pdo = $this->pdo;
        $sql = "SELECT * FROM acta WHERE ID = :id";
        $prepared = $pdo->prepare($sql);
        $resultQuery = $prepared->execute([
            'ID' => $id
            ]);
